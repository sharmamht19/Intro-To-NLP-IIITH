...
#         print(self.nGramContext[1])
        unigram_context, self.unigram_counter = generate_ngram_model(1, corpus_path, self.tokenizer)
        ...
        if self.smoothing_type == "g":
            ...
            smoothing_instance = LinearInterpolationSmoothing(self.nGramCounter, bigram_counter, self.unigram_counter, is_train=True, lambdas=self.lambdas)
    ...
    def calculate_probability(self, sentence):
        ...
        if( self.n_gram_order - 2 > 0 ):
            tokenized_sentence = ["<sos>"] * (self.n_gram_order - 2) + tokenized_sentence
...
class Generator:
    ...
    def get_ngram(self,sentence):
        tokenized_sentence = self.language_model.tokenizer.sentence_tokenizer(sentence)
        if(len(tokenized_sentence)<3):
            tokenized_sentence = ["<sos>"] + tokenized_sentence
        # Get the second last word
        return tokenized_sentence[-3],tokenized_sentence[-2]
    
    def generate_candidates(self,sentence, k):
        
        word1, word2 = self.get_ngram(sentence)
        
        # Ensure the language model is trained
        if self.language_model.probs is None:
            raise ValueError("Language model is not trained. Train the model before generating candidates.")

        # Form the context for the given words
        context = f"{word1} {word2}" if word1 and word2 else word2
        # Initialize an empty dictionary to store candidates
        candidate_ngrams = {}
        # Iterate over all words in the vocabulary
        for word3 in self.language_model.vocabulary:
            if word3 in ["<eos>", "<sos>"]:
                continue
            # Form the n-gram
            n_gram = f"{context} {word3}" if context else word3

            # Get the probability using the smoothing instance
            probability = self.language_model.smoothing_instance.get_probability(n_gram)

            # Add the word and probability to the candidate dictionary
            candidate_ngrams[n_gram] = probability
            
        # Sort the candidate N-grams by probability in descending order
        sorted_candidates = sorted(candidate_ngrams.items(), key=lambda x: x[1], reverse=True)

        # Take the top k candidates
        top_k_candidates = sorted_candidates[:k]

        # Return the candidates as a dictionary
        candidate_dict = {word: prob for word, prob in top_k_candidates}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        generator = Generator(language_model=lm_model)
        candidates = generator.generate_candidates(input_sentence, k)
        print()
