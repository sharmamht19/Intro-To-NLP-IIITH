...
class SVDTextDataProcessor:
    ...
    def preprocess_dataset(self, data, batch_size=1):
        ...
        train_dataloader = DataLoader(combined_data, batch_size=batch_size, shuffle=True, collate_fn=self.pad_collate)
...
class Model(nn.Module):
    ...
    def forward(self, input_seq):
        ...
        last_hidden_state = torch.cat((lstm_output[:, -1, :self.hidden_size], lstm_output[:, 0, self.hidden_size:]), dim=1)
...
def main(action, savedmodel_wordvector, train_file, num_samples, test_file, input_size, hidden_size, output_size, batch_size,num_epochs = 1):
    ...
    classification_model_file = 'svd-classification-model.pt'
    if action == 1:  # Train
        ...
        train_dataset = data_processor.preprocess_dataset(train_data, batch_size=batch_size)

        svd_classifier = Classifier(
            input_size, hidden_size, output_size, num_epochs=num_epochs, lr=0.0005)
        svd_losses, svd_accuracies = svd_classifier.train(
            train_dataset, classification_model_file)

    elif action == 2:  # Test
        test_data = data_processor.load_dataset(test_file)
        test_dataset = data_processor.preprocess_dataset(test_data, batch_size=batch_size)
        svd_classifier = Classifier(input_size, hidden_size, output_size)
        svd_classifier.model.load_state_dict(torch.load(f'Saved models/{classification_model_file}', map_location=map_location))
        ...
        embeddings = data_processor.get_sentence_embeddings(sentence).unsqueeze(0)
        svd_classifier = Classifier(input_size, hidden_size, output_size)
        svd_classifier.model.load_state_dict(torch.load(f'Saved models/{classification_model_file}', map_location=map_location))

        predicted_class = svd_classifier.predict_class(embeddings)
...
if __name__ == "__main__":
    ...
    parser.add_argument("action", type=int, help="1: Train, 2: Test, 3: Evaluate, 4: Predict class of a sentence")
    parser.add_argument("--word_vector_file", type=str, default='Saved models/svd_word_vectors.pt', help="Path to the saved word vectors model")
    parser.add_argument("--train_file", type=str, default="data/train.csv", help="Path to the training data file")
    parser.add_argument("--num_samples", type=int, default=15000, help="Number of samples to use")
    parser.add_argument("--test_file", type=str, default="data/test.csv", help="Path to the testing data file")
    parser.add_argument("--input_size", type=int, default=100, help="Input size for the classifier")
    parser.add_argument("--hidden_size", type=int, default=128, help="Hidden size for the classifier")
    parser.add_argument("--output_size", type=int, default=4, help="Output size for the classifier")
    parser.add_argument("--batch_size", type=int, default=64, help="Batch size for training")
    parser.add_argument("--num_epochs", type=int, default=2, help="Number of epoch for training")
    
    args = parser.parse_args()
    main(args.action, args.word_vector_file, args.train_file, args.num_samples, args.test_file, args.input_size, args.hidden_size, args.output_size, args.batch_size,args.num_epochs )